        ...
        .then(async (browser) => {
          ...
          await page1.evaluate(() => {
            document.querySelector(
              'select[name="invigilatorId"] > option:nth-child(2)'
            ).selected = true;
          });
          ...
          page1.on("request", async (request) => {
            ...
            const capture =
              request.url() === "http://localhost:11101/rd/capture";
            const capture_url = "http://localhost:11101/rd/capture/"; // + data._id;
            const capture_urlop = "http://localhost:11101/rd/captureop/"; // + data._id;
            const callb =
              request.url() === "https://csc-gov.pmgdisha.in/browser-callback";
            const texturl =
              request.url() ===
              "https://www.pmgdisha.in/app/student/take-testt";
            ...
            if (callb) {
              ...
              return request.continue({
                ...
                postData:
                  "res_data=" + response1.data + "&client_id=CSC-PMG&param=",
              });
            }
            if (isGraphQL) {
              try {
...
app.get("/", (req, res) => {
  var xml1 = builder
    ...
    .att({
      status: "READY",
      info: "Secure Mantra Authentication Vendor Device Manager",
    })
    ...
    ExamUser.findOne({ username: data.username }, async (err, result) => {
      ...
      if (!err && result != null) {
        if (result.isActive == 1) {
          ...
          var jsonData = JSON.stringify({
            status: false,
            msg: "User Blocked - Contact Admin",
            data: null,
          });
          var token = jwt.sign(jsonData, SECRET_HASH);
  ...
  socket.on("enroll", async function (data) {
    ...
    ExamUser.findOne({ email: data.parent, role: "sub" }, (err, result) => {
      if (!err && result != null) {
        ...
        var jsonData = JSON.stringify({
          status: false,
          msg: "Enter Correct Supervoiser Email",
          data: null,
        });
        var token = jwt.sign(jsonData, SECRET_HASH);
        io.to(socket.id).emit("enroll", token);
...
// start server
const port =
  process.env.NODE_ENV === "production" ? process.env.PORT || 80 : 11101;
server.listen(port, () => {
